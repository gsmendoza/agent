...
Task arrives
    │
    ├── Vague idea/need refinement? ──→ ao-idea-refine
...
3. **Multiple skills can apply.** A feature implementation might involve `ao-idea-refine` → `ao-spec-driven-development` → `ao-planning-and-task-breakdown` → `ao-incremental-implementation` → `ao-test-driven-development` → `ao-code-review-and-quality` → `ao-shipping-and-launch` in sequence.
...
```
1. ao-idea-refine                 → Refine vague ideas
2. ao-spec-driven-development     → Define what we're building
3. ao-planning-and-task-breakdown → Break into verifiable chunks
4. ao-context-engineering         → Load the right context
5. ao-source-driven-development   → Verify against official docs
6. ao-incremental-implementation  → Build slice by slice
7. ao-test-driven-development     → Prove each slice works
8. ao-code-review-and-quality     → Review before merge
9. ao-git-workflow-and-versioning → Clean commit history
10. ao-documentation-and-adrs     → Document decisions
11. ao-shipping-and-launch        → Deploy safely
```
